...
$runtimeCases = @(
    ...
    @{ Name = 'Whitted swapchain resize'; Arguments = @('--integrator', 'whitted', '--frames', '90', '--resize-test', '--shadow', 'physical') },
...
foreach ($runtimeCase in $runtimeCases)
{
    ...
    $runtimeOutput = & $executablePath @($runtimeCase.Arguments) 2>&1
