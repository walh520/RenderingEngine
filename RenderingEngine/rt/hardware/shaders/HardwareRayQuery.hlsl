    ...
    out bool samplerMappingValid)
{
    ...
    uint geometryIndex = CanonicalGeometryIndex(instanceIndex, query.CandidateGeometryIndex());
    GpuGeometryV0 geometry = gGeometries[geometryIndex];
    ...
    GpuVertexV0 v1;
    GpuVertexV0 v2;
    FetchTriangle(geometryIndex, query.CandidatePrimitiveIndex(), ignoredIndices, v0, v1, v2);
...
void RayQueryMain(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    ...
    while (query.Proceed())
    {
        if (query.CandidateType() == CANDIDATE_NON_OPAQUE_TRIANGLE)
        {
            ...
            if (IsCandidateAccepted(query, forceOpaque, candidateMappingValid))
            {
    ...
    {
        gHits[rayIndex] = MakeCommittedHit(query, ray, forceOpaque);
    }
    else
    {
        gHits[rayIndex] = MakeMiss(ray);
